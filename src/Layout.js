import React, { useState } from 'react';
import './App.css';


export const Header = ({ showName }) => {

    const handleClick = () => {
        window.location.href = '/'; // Przekierowanie na stronę główną po kliknięciu
    };


    return (
        <header className="header">
            <div className="logo-container" onClick={handleClick}>
                <svg className="logo" height="50" width="35"
                    viewBox="-1 0 20 20"
                    version="1.1"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="#ffffff"
                >
                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                    <g id="SVGRepo_iconCarrier">
                        <title>html [#ffffff]</title>
                        <desc>Created with Sketch.</desc>
                        <defs> </defs>
                        <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                            <g id="Dribbble-Light-Preview" transform="translate(-61.000000, -7639.000000)" fill="#ffffff">
                                <g id="icons" transform="translate(56.000000, 160.000000)">
                                    <path d="M19.4350881,7485 L19.4279481,7485 L10.8119794,7485 L11.0180201,7487 L19.2300674,7487 C19.109707,7488.752 18.7455658,7492.464 18.6119454,7494.153 L13.99949,7495.451 L13.99949,7495.455 L13.98929,7495.46 L9.37377458,7493.836 L9.05757353,7490 L11.3199411,7490 L11.4800816,7492.063 L13.99337,7493 L13.99949,7493 L16.5086984,7492.1 L16.7667592,7489 L8.95659319,7489 C8.91885306,7488.599 8.43333144,7483.392 8.34867116,7483 L19.6370488,7483 C19.5738086,7483.66 19.5095484,7484.338 19.4350881,7485 L19.4350881,7485 Z M5,7479 L6.63812546,7497.148 L13.98929,7499 L21.3598345,7497.111 L23,7479 L5,7479 Z" id="html-[#ffffff]">
                                    </path>
                                </g>
                            </g>
                        </g>
                    </g>
                </svg>

            </div>

            <div className="header-text-container">
                <h1 className="header-text">Zadanie rekrutacyjne</h1>
                {showName && <div className="name-display">Katarzyna Czarnota</div>}
            </div>
        </header >
    );
};


export const Footer = ({ handleShowName }) => {

    const [isFrameVisible, setIsFrameVisible] = useState(false);

    const toggleFrame = () => {
        setIsFrameVisible(!isFrameVisible);
    };

    const handleClick = () => {
        window.location.href = '/'; // Przekierowanie na stronę główną po kliknięciu
    };



    return (
        <footer className="footer">

            <div className="footer-logo">
                <div className="square"></div>
                <div className="textfooter-logo">
                    <span >CSS</span>
                    <br />
                    <span >IS</span>
                    <br />
                    <span >AWESOME</span>
                </div>
            </div>

            <div className="textfooter-container">
                <div className="linefooter"></div>
                <p className="textfooter">nabthat</p>
                <div className="linefooter"></div>
            </div>

            {/*sliding up frame */}
            <div className="buttonFooter-container">
                {/* togle button */}

                <button className="buttonfooter" onClick={toggleFrame}>
                    POKAŻ {isFrameVisible ? <i className="arrow down"></i> : <i className="arrow up"></i>}
                </button>
               
            </div>

            {/* Conditional rendering of the frame */}
            {isFrameVisible && (
                <div id="frame" className="frame">
                    <button className="reset-button" onClick={handleClick}> <i class="arrow right"></i> ZRESETUJ USTAWIENIA </button>
                    <button className="nameDisplay-button" onClick={handleShowName}> <i class="arrow right"></i> POKAŻ DANE OSOBOWE </button>
                </div>
            )}
         </footer>
    );
};
